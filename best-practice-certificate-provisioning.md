...
To enable zero-configuration TLS Certificate provisioning, manufacturers must include a unique TLS Client Certificate on all their devices and provide the corresponding Root Certificate Authority to customers. If the TLS Client Certificate or the chain of trust is compromised the manufacturer must revoke the comprised certificates.
